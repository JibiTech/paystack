...
class PaystackGatewayReturn
{
	...
	public function getResponse(){
		...
		//check if its demo
		if($this->_pluginparams->get('paystack_live_demo')==0)
		{
			 $secret_key = $this->_pluginparams->get('paystack_test_secret_key');
		 }
		else {
			 $secret_key = $this->_pluginparams->get('paystack_live_secret_key');
		 }
		...
		if (
			(!property_exists($transData, 'error')) &&
			 ...
			 (strpos($transData->reference, $transactionid) === 0)
			 )
		{
